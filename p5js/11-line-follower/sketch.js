const POPULATION = 50
const MUTATION_RATE = 0.05
const START_ANGLE = 5

let track
let vehicles = []
let startPosition
let maxScore = 0


function setup() {
	angleMode(DEGREES)
	createCanvas(1000, 700)
	track = new Track()
	startPosition = createVector(width / 2, height / 2).add(createVector(1, 0).rotate(START_ANGLE).setMag(295))
	for (let i = 0; i < POPULATION; i++) {
		vehicles.push(new Vehicle(startPosition.x, startPosition.y))
	}
}

function getParents() {
	return vehicles.sort((a, b) => b.score - a.score).slice(0, 10)
}

function getParent() {
	let parents = vehicles.sort((a, b) => b.score - a.score)
	let sumScore = parents
		.reduce((sum, p) => sum + p.score * 100, 0)
	let val = random(sumScore)
	for (let p of parents) {
		if (val < p.score * 100) {
			return p
		}
		else val -= p.score * 100
	}
}

function reborn() {
	let newVehicles = []
	for (let i = 0; i < POPULATION; i++) {
		let parent = getParent()
		newVehicles.push(new Vehicle(startPosition.x, startPosition.y, parent))
	}
	vehicles = newVehicles
}

function draw() {
	background(0)
	track.show()
	for (let vehicle of vehicles) {
		if (vehicle.alive) {
			vehicle.update()
			vehicle.show(track.walls)
		}
	}
	let len = vehicles.filter(v => v.alive).length
	let tempScore = max(vehicles.map(v => v.score))
	if (len === 0) {
		if(tempScore > maxScore) {
			maxScore = tempScore
		}
		reborn()
	}
	push()
	translate(width / 2, height / 2)
	textAlign(CENTER, CENTER)
	fill(255)
	textSize(40)
	text(len, 0, -20)
	textSize(20)
	text(`Record : ${maxScore.toFixed(2)}`, 0, 20)
	let x = createVector(1, 0).setMag(350).rotate(maxScore)
	fill(255, 200)
	line(0,0,x.x,x.y)
	pop()
}